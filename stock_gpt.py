...
FAKE_NEWS = [
    "[91m[突发][0m 央行宣布降准0.25个百分点，释放长期资金约5000亿",
    "[91m[重磅][0m 证监会：支持头部券商做优做强，适时推出做市商制度",
    "[91m[快讯][0m 商务部：将采取更大力度举措稳外资，扩大开放领域",
    "[91m[要闻][0m 国资委：推动国有控股上市公司价值回归合理区间",
    "[91m[独家][0m 知情人士：新能源汽车购置税减免政策将延续至2027年",
]

GUA_NAMES = ["山天大壮","地水师","火天大有","雷水解","水风井","天泽履","风火家人"]
...
def header():
    ...
    print(f"{C_BOLD}{C_CYAN}  StockGPT Terminal  {now.strftime('%Y-%m-%d %H:%M:%S').rjust(30)}{C_RESET}")
    print(f"{C_BOLD}{C_CYAN}={'=' * 60}={C_RESET}")
...
def menu():
  ...
  {C_WHITE}|  {C_YELLOW}[6]{C_RESET} 股民投诉窗口（AI负责挨骂）              |{C_RESET}
  {C_WHITE}|  {C_GREEN}[7]{C_RESET} 玄学选股（赛博算命）                     |{C_RESET}
...
def loading(text="思考中"):
    for i in range(3):
        print(f"\r{C_YELLOW}{text}{'.' * (i+1)}{' ' * (2-i)}{C_RESET}", end="", flush=True)
        time.sleep(0.35)
    print(f"\r{C_GREEN}[OK]{C_RESET} ")
...
def daily_analysis():
    ...
    for name, color in sectors:
        bar = random.choice(["[92m▓▓▓▓░░[0m","[91m▓▓░░░░[0m","[92m▓▓▓▓▓▓[0m"])
        pct = random.uniform(-3, 5)
        arrow = C_GREEN+"▲" if pct>0 else C_RED+"▼"
        print(f"  {name:<12} {bar} {arrow}{abs(pct):.1f}%")

    divider()
    print(f"\n  {C_BOLD}AI今日观点：{C_RESET}")
    loading("分析市场")
    opinions = [
        (C_GREEN, "市场今日表现强劲，AI算力板块延续升势，建议关注低位补涨的半导体个股，注意控制仓位。"),
        (C_YELLOW, "指数震荡分化，AI题材进入轮动期，前期涨幅较大的个股注意获利了结，可适当布局超跌消费电子。"),
        (C_RED, "市场情绪偏谨慎，防御板块相对抗跌，建议保持轻仓，等待市场企稳后再考虑加仓。"),
        (C_CYAN, "创业板指走势明显强于主板，市场风格偏向成长，科创50可关注，短期趋势向好。"),
    ]
    color, opinion = random.choice(opinions)
    print(f"\n  {color}{opinion}{C_RESET}\n")
...
def stock_diagnosis():
    ...
    print(f"  {C_WHITE}今日价格：{color}{pct:+.2f}%{C_RESET}  当前价 ¥{price:.2f}")
    print(f"  {C_WHITE}市值：{market_cap}  PE：{pe:.1f}  PB：{pb:.1f}")
    print(f"  {C_WHITE}营收增长：{C_GREEN if revenue_growth > 0 else C_RED}{revenue_growth:+.1f}%{C_RESET}  股东户数：{holder_num}万")
    ...
    color = C_GREEN if "推荐" in verdict or "布局" in verdict else (C_YELLOW if "观望" in verdict else C_RED)

    print(f"\n  {C_BOLD}AI诊断结果：{color}{verdict}{C_RESET}")
    ths = BULL_THS if "推荐" in verdict or "布局" in verdict else BEAR_THS
    print(f"\n  {C_WHITE}诊断依据：{C_RESET}")
    for _ in range(3):
        print(f"  {C_DIM}• {random.choice(ths)}{C_RESET}")

    print(f"\n  {C_YELLOW}附加情报：{C_RESET}")
    print(f"  {C_DIM}  {random.choice(FAKE_NEWS)}{C_RESET}")
    print(f"  {C_YELLOW}  {random.choice(RUMORS)}{C_RESET}")

    print(f"\n  {C_BOLD}AI补充判断：{C_RESET}")
    print(f"  {C_CYAN}  「{random.choice(FRUSTRATING_QUOTES)}」{C_RESET}")
...
def tomorrow_prediction():
    ...
    print(f"\n  {C_BOLD}预测日期：{tomorrow.strftime('%Y年%m月%d日')}（星期" + ["一","二","三","四","五","六","日"][tomorrow.weekday()] + "）{C_RESET}")
    print(f"  {C_WHITE}上证指数预测：{C_GREEN if index_pred>0 else C_RED}{index_pred:+.2f}%{C_RESET}")
    print(f"  {C_WHITE}两市成交额预测：{random.randint(7000, 14000):,}亿{C_RESET}")
    ...
    cur_price = random.uniform(20, 300)
    target_price = cur_price * random.uniform(1.05, 1.5)
    print(f"\n  {C_BOLD}🎯 明日金股（仅供演示）：{C_RESET}")
    print(f"  {C_WHITE}  {stock}（{code}）{C_RESET}")
    print(f"  {C_WHITE}  今日收盘：¥{cur_price:.2f}  →  明日目标价：¥{target_price:.2f}（{((target_price/cur_price)-1)*100:+.1f}%）{C_RESET}")

    insane = random.choice(INSANE_PREDICTIONS)
    print(f"\n  {C_MAG}大胆预测：{C_RESET}")
    print(f"  {C_WHITE}  「{insane[0]}」{C_RESET}")
    print(f"  {C_DIM}  预测时间：{insane[1]}  置信度：{insane[2]*100:.0f}%（瞎编的）{C_RESET}")
...
def market_news():
    ...
    for i, n in enumerate(random.sample(FAKE_NEWS, k=4), 1):
        print(f"  {C_BOLD}{i}.{C_RESET} {n}")
        time.sleep(0.25)

    print(f"\n  {C_YELLOW}🗣️  市场传言：{C_RESET}")
    print(f"  {C_DIM}  {random.choice(RUMORS)}{C_RESET}")
    ...
    anns = [
        ...
        f"【异动公告】{stock}（{code}）连续3个交易日内日收盘价格涨幅偏离值累计达到20%，特此公告。",
        f"【业绩预告】{stock}（{code}）预计2026年一季度归母净利润同比+{random.uniform(20,80):.0f}%。",
        f"【澄清公告】近日有媒体报道公司涉及{tag}业务，经核实，该报道不属实。",
    ]
...
def famous_quotes():
    clear()
    header()
    print(f"\n  {C_BOLD}{C_MAG}名言警句{C_RESET}")
    divider()
    print(f"  {C_DIM}亏钱的时候读一读，很治愈{C_RESET}\n")

    for i, (quote, author) in enumerate(FAMOUS_QUOTES, 1):
...
def complaint_window():
    clear()
    header()
    print(f"\n  {C_BOLD}{C_YELLOW}股民投诉窗口{C_RESET}")
    divider()
    ...
    responses = [
        ...
        "非常抱歉给您带来的体验。我们AI已经学习了10000小时，水平还是这样，我们会继续努力。",
    ]
    print(f"\n  {C_BOLD}StockGPT 客服回复：{C_RESET}")
...
def mystic_stock_picker():
    ...
    print(f"""
  {C_YELLOW}  卦象显示：{random.choice(GUA_NAMES)}{C_RESET}
  ...
  {C_BOLD}{C_GREEN}  推荐股票：{stock}（{code}）{C_RESET}
  {C_WHITE}  所属概念：{tag}{C_RESET}
  {C_WHITE}  综合评分：{random.choice(stars)} {random.randint(60, 98)}分{C_RESET}
  {C_WHITE}  操作建议：{C_RED if action in ['减仓','清仓'] else C_GREEN}{action}{C_RESET}
  {C_WHITE}  预计波动：{C_GREEN if random.uniform(0,1)>0.4 else C_RED}{random.uniform(-5, 9.9):+.1f}%{C_RESET}

  {C_BOLD}{C_MAG}  赛博占卜赠言：{C_RESET}
  {C_CYAN}  「{random.choice(['持股待涨，不动如山','逢高减磅，落袋为安','静待时机，不可盲目','分批建仓，稳中求进','管住手，别乱动'])}」{C_RESET}
""")

    input(f"  {C_CYAN}[回车返回菜单]{C_RESET}")
